...
/**************************************************************/
static int
win32_flush(/*@unused@*/ hio_t* hio) { /* win32_flush is intentionately blank */
  return (RET_OK);
}

/**************************************************************/
static int
win32_poweroff(/*@unused@*/ hio_t* hio) { /* win32_poweroff is intentionately blank */
  return (RET_OK);
}
...
int hio_win32_probe(const dict_t* config,
  ...
  if (tolower(path[0]) == 'h' && tolower(path[1]) == 'd' && tolower(path[2]) == 'd' && isdigit(path[3]) && ((path[4] == ':' && path[5] == '\0') || (isdigit(path[4]) && path[5] == ':' && path[6] == '\0')))
    result = RET_OK;
  else
...
#if defined(_BUILD_UNIX)
    if (result == RET_ERR || result == RET_BAD_DEVICE)
      result = RET_NOT_COMPAT;
#endif
    if (result == OSAL_OK) {
      osal_handle_t device;
