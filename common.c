...
copy_data(osal_handle_t in,
  ...
  if (buffer != NULL) {
    ...
    u_int32_t len;
    do {
      u_int32_t chunk_size = (copy_til_eof ? buff_size : ((u_int64_t) buff_size < bytes) ? buff_size
                                                                                         : (u_int32_t) bytes);
...
int dump_device(const dict_t* config,
  ...
  if (result == OSAL_OK) {
    ...
    if (result == OSAL_OK) {
      ...
      if (max_size > 0 && size_in_bytes > max_size)
      ...
      if (result == OSAL_OK) {
        ...
        while (result == RET_OK && offset < size_in_bytes) {
          ...
          const u_int32_t count =
              (u_int32_t) (remaining > BYTES_AT_ONCE ? BYTES_AT_ONCE : remaining);
