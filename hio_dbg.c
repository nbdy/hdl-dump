...
remap_sector(const hio_dbg_t* dbg,
  ...
  if ((start_sector + num_sectors) / 2 > dbg->size)
    ...
    *len = (num_sectors <= 2 ? num_sectors : 2) * HDD_SECTOR_SIZE;
    return (1);
  } else if (start_sector > HDL_HEADER_OFFS_IN_SECT && !((start_sector - HDL_HEADER_OFFS_IN_SECT) % IN_SECT_128MB)) { /* start sector multiplies on HD Loader game header */
    *offset = ((start_sector - HDL_HEADER_OFFS_IN_SECT) / IN_SECT_128MB) * 2048 + 1024;
    *len = (num_sectors <= 2 ? num_sectors : 2) * HDD_SECTOR_SIZE;
    return (1);
  }
  return (0);
...
dbg_read(hio_t* hio,
  ...
  if (remap_sector(dbg, start_sector, num_sectors, &offset, &total_bytes) && total_bytes > 0 && fseek(dbg->out, offset, SEEK_SET) == 0) {
