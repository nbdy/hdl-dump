...
svr_read(const udp_frame_t* uf,
  ...
  if ((rd_sector % rd_buf_size) == 0 && /* buffer contains any data */
      start >= rd_sector && start + count <= rd_sector + rd_buf_size)
...
svr_write(const udp_frame_t* uf,
          ...
          unsigned char count,
          const void* buf) {
  if ((start >= rd_sector && start <= rd_sector + rd_buf_size) || (start + count >= rd_sector && start + count <= rd_sector + rd_buf_size))
    rd_sector = (unsigned long) -1; /* invalidate read cache */

  if ((wr_sector % wr_buf_size) == 0 && /* buffer contains any data */
      start >= wr_sector && start + count <= wr_sector + wr_buf_size)
