...
udp_packet(const udp_frame_t* uf) {
  ...
  if (NT_IP_EQ(uf->ip.dst_ip, my_ip) && GET_NT_WORD(uf->udp.len) <= MAX_UDP_DATA_LEN) {
...
int udp_probe(const void* frame, unsigned int frame_len) {
  ...
  if (frame_len >= sizeof(udp_frame_t)) {
    ...
    if (IS_NT_WORD(uf->eth.eth_type, 0x0800) && uf->ip.version_and_hdr_len == ((4 << 4) | (5 << 0)) && uf->ip.diff_serv == 0 && uf->ip.proto == 17 /* UDP */ && (GET_NT_WORD(uf->ip.flags_frag_offs) == 0x0000 || GET_NT_WORD(uf->ip.flags_frag_offs) == 0x4000 /* don't fragment */)) {
      result = 1;
    }
  }
