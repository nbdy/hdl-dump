...
isofs_find_pvd_addr(iin_t* iin,
  ...
  *system_id = *volume_id = '\0';
  ...
  if (layer == 1) {
    start_sector = ((u_int32_t) buffer[83] << 24 | (u_int32_t) buffer[82] << 16 | (u_int32_t) buffer[81] << 8 | (u_int32_t) buffer[80] << 0);
  ...
  if (result == OSAL_OK) {
    if (buffer[0] == 0x01) /* primary volume descriptor set */
    {
      ...
      *ptr_start_addr = (u_int64_t) (buffer[143] << 24 | buffer[142] << 16 | buffer[141] << 8 | buffer[140] << 0) * (u_int64_t) CDVD_SECT_SIZE;
      if (system_id != NULL) {
  ...
  *root_start_addr = 0;
  if (result == OSAL_OK) {
    ...
    do {
      ...
      id_len = *p++;
      if (id_len == 0 || p > buffer + 2047)
        break; /* last entry */
      ext_len = *p++;
      dir_start_addr = ((u_int32_t) p[3] << 24 | (u_int32_t) p[2] << 16 | (u_int32_t) p[1] << 8 | (u_int32_t) p[0] << 0);
      p += 4;
  ...
  *file_start_addr = *file_length = 0;
  if (result == OSAL_OK) {
    ...
    do {
      ...
      len_dr = *p++;
      if (len_dr == 0 || p > buffer + 2047)
        break; /* last entry */
      len_ext_ar = *p++;
      start_addr = ((u_int32_t) p[3] << 24 | (u_int32_t) p[2] << 16 | (u_int32_t) p[1] << 8 | (u_int32_t) p[0] << 0);
      p += 8;
      length = ((u_int32_t) p[3] << 24 | (u_int32_t) p[2] << 16 | (u_int32_t) p[1] << 8 | (u_int32_t) p[0] << 0);
      p += 8;
