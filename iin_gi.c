...
int iin_gi_probe_path(const char* path,
  ...
  if (result == OSAL_OK) {
    ...
    if (result == OSAL_OK)
      result = (header[0x00] == 0xda && header[0x01] == 0xda && header[0x02] == 0xfe && header[0x03] == 0xfe) ? OSAL_OK : RET_NOT_COMPAT;

#if defined(CHECK_11111111)
    if (result == OSAL_OK)
      result = (header[0x14] == 0x11 && header[0x15] == 0x11 && header[0x16] == 0x11 && header[0x17] == 0x11) ? OSAL_OK : RET_NOT_COMPAT;
#endif /* CHECK_11111111 defined? */

    if (result == OSAL_OK) { /* check whether image is one or multiple parts */
      if (header[0x62] == 0x22 && header[0x63] == 0x22 && header[0x64] == 0x22 && header[0x65] == 0x22)
        single_file = 1;
      else if (header[0x62] == 0x88 && header[0x63] == 0x88 && header[0x64] == 0x88 && header[0x65] == 0x88)
        single_file = 0;
      else
    ...
    if (result == OSAL_OK) { /* mode1/mode2? */
      ...
      if (result == OSAL_OK) { /* total number of sectors */
        num_sectors = (header[0x37] << 24 | header[0x36] << 16 | header[0x35] << 8 | header[0x34]);
        result = (num_sectors == (header[0x3b] << 24 | header[0x3a] << 16 | header[0x39] << 8 | header[0x38]) && num_sectors == (header[0x7d] << 24 | header[0x7c] << 16 | header[0x7b] << 8 | header[0x7a])) ? OSAL_OK : RET_BAD_COMPAT;
      }
    }
    ...
    if (result == OSAL_OK) {
      if (single_file)
        ...
        for (i = 0; i < num_files; ++i) {
          ...
          char source[260];
          u_int32_t length_s = (header[FILE_SIZES_OFFSET[i] + 3] << 24 | header[FILE_SIZES_OFFSET[i] + 2] << 16 | header[FILE_SIZES_OFFSET[i] + 1] << 8 | header[FILE_SIZES_OFFSET[i]]);
