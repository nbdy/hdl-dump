...
thd_read(iin_t* iin,
  ...
  do {
    ...
    if (result == 0) { /* worker is idle */
      ...
      if (start_sector == buf->start_sector && num_sectors == buf->num_sectors) { /* data is currently available */
        *data = buf->buf;
        *length = buf->bytes;
        result = buf->result;
        break;
  ...
  if (result == RET_OK && num_sectors == IIN_NUM_SECTORS && start_sector + num_sectors < thd->total_sectors) { /* schedule pre-fetch for the next group of sectors */
