...
is_apa_partitioned(osal_handle_t handle) {
  ...
  if (result == OSAL_OK) {
    if (bytes == sizeof(part) && memcmp(part.magic, PS2_PARTITION_MAGIC, 4) == 0 && get_u32(&part.checksum) == apa_partition_checksum(&part))
      return (RET_OK); /* APA */
    else
...
/*@sets toc->slice[slice_index]@*/
{
  ...
  if (toc->got_2nd_slice)
    if (slice_index == 0)
      total_sectors = (toc->size_in_kb < ALMOST_128MB ? toc->size_in_kb : ALMOST_128MB) * 2; /* 1st */
    else
  ...
  do { /* read partitions */
    result = hio->read(hio, sector + SLICE_2_OFFS * slice_index, 2,
                       &part, &bytes);
    if (result == RET_OK && bytes == sizeof(part)) {
      if (memcmp(part.magic, PS2_PARTITION_MAGIC, 4) != 0)
        result = RET_NOT_APA;
      if (result == RET_OK && get_u32(&part.checksum) == apa_partition_checksum(&part)) { /* NOTE: start should be the same as sector? */
        u_int32_t start = get_u32(&part.start);
        if (start < total_sectors && start + get_u32(&part.length) < total_sectors) {
          ++count;
  ...
  do {
    ...
    for (i = 0; i < *partitions_used - 1; ++i) {
      u_int32_t curr_part_end_sector =
          partitions[i].sector + partitions[i].size_in_mb * ((1 _MB) / 512);
      ...
      if (u_int32_to_be <= max_part_size_in_mb && curr_part_end_sector == partitions[i + 1].sector && next_is_same_size && would_be_aligned) {
    ...
    if (partitions != NULL) {
      ...
      if (result == RET_OK) { /* create new partitions in the partition slice */
        ...
        u_int32_t last_partition_start =
            slice->part_count > 0 ? get_u32(&slice->parts[slice->part_count - 1].header.start) : 0;
  ...
  if (result == RET_OK) {
    ...
    for (i = 0; result == RET_OK && i < slice->part_count; ++i) {
      if (slice->parts[i].modified) {
        ...
        u_int32_t bytes;
        const ps2_partition_header_t* part = &slice->parts[i].header;
        u_int32_t sector = (get_u32(&part->start) + SLICE_2_OFFS * slice_index);
...
apa_list_problems(const apa_slice_t* slice,
                  /*@out@*/ char* buffer,
                  size_t buffer_size) { /* NOTE: keep in sync with apa_check */
  u_int32_t i, j, k;
  char tmp[1024];
  size_t len;
  ...
  *buffer = '\0';
  for (i = 0; i < slice->part_count; ++i) {
    ...
    if (get_u32(&part->start) < total_sectors && get_u32(&part->start) + get_u32(&part->length) <= total_sectors)
      ;
    else {
    ...
    if (get_u32(&part->main) == 0 && get_u16(&part->flags) == 0 && get_u32(&part->start) != 0) { /* check sub-partitions */
      u_int32_t count = 0;
...
/**************************************************************/
static int
apa_check_slice(const apa_slice_t* slice) { /* NOTE: keep in sync with apa_list_problems */
  u_int32_t i, j, k;
  ...
  for (i = 0; i < slice->part_count; ++i) {
    ...
    apa_partition_t* curr = slice->parts + i;
    apa_partition_t* next = slice->parts + (i + 1 < slice->part_count ? i + 1 : 0);
    if (get_u32(&curr->header.prev) != get_u32(&prev->header.start) || get_u32(&curr->header.next) != get_u32(&next->header.start))
