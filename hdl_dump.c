...
show_slice_map(const apa_slice_t* slice) {
  ...
  for (i = 0; i < slice->total_chunks; ++i) {
    if (count == 0)
      fprintf(stdout, "%3uGB: ",
              (unsigned int) ((i / ((GIGS_PER_ROW * 1024) / 128)) * GIGS_PER_ROW));
  ...
  if (result == RET_OK && hio != NULL) {
    ...
    if (result == RET_OK && glist != NULL) {
      ...
      printf("total %uMB, used %uMB, available %uMB\n",
             ...
             (unsigned int) ((glist->total_chunks - glist->free_chunks) * 128),
      ...
      if (result == OSAL_OK) {
        ...
        if (new_style == 0)
          ...
          fprintf(out, "%s%s %luKB \"%s\" \"%s\" \n",
                  (info.layer_pvd != 0 ? "dual-layer " : ""),
                  (info.media_type == mt_cd ? "CD" : info.media_type == mt_dvd ? "DVD"
                                                                               : "?"),
                  (unsigned long) (tot_size / 1024),
      ...
      if (result == RET_OK) {
        ...
        for (i = 0; i < dlist->used; ++i) {
          if (dlist->device[i].type == 0x05) { /* MMC device */
            ...
            printf("%lu MB\n",
                   (unsigned long) (((u_int64_t) dlist->device[i].size_in_sectors * dlist->device[i].sector_size) / (1024 * 1024)));
          }
        }
  ...
  if (result == OSAL_OK && iin1 != NULL) {
    ...
    if (result == OSAL_OK && iin2 != NULL) {
      ...
      if (result == OSAL_OK) {
        ...
        if (sector_size1 == IIN_SECTOR_SIZE && sector_size2 == IIN_SECTOR_SIZE)
      ...
      while (result == OSAL_OK && len1 / IIN_SECTOR_SIZE > 0 && len2 / IIN_SECTOR_SIZE > 0) {
        u_int32_t sectors1 = (num_sectors1 > IIN_NUM_SECTORS ? IIN_NUM_SECTORS : num_sectors1);
        result = iin1->read(iin1, sector, sectors1, &data1, &len1);
        if (result == OSAL_OK) {
          u_int32_t sectors2 = (num_sectors2 > IIN_NUM_SECTORS ? IIN_NUM_SECTORS : num_sectors2);
          ...
          if (result == OSAL_OK) {
            ...
            if (len1 != len2 && (len1 == 0 || len2 == 0)) { /* track which file is longer */
              if (len2 == 0)
                longer_file = 1;
              else if (len1 == 0)
                longer_file = 2;
      ...
      if (out != NULL) {
        ...
        while (sector < sectors) {
          ...
          if (result == RET_OK && bytes == 1024)
            result = (fwrite(buf, 1, 1024, out) == 1024 ? RET_OK : RET_ERR);
          if (result == RET_OK) {
      ...
      if (result == RET_OK && hio != NULL) {
        ...
        if (result == RET_OK) {
          ...
          if (result == RET_OK || incompatible == 1) {
            ...
            result = hdl_inject(hio, iin, &game, slice_index, is_hidden, pgs);
            result = (result == RET_OK && incompatible == 1 ? RET_DDB_INCOMPATIBLE : result);
          }
        }
        (void) hio->close(hio), hio = NULL;
      }
...
copy_hdd(const dict_t* config,
  ...
  if (result == RET_OK) { /* calculate space required */
    ...
    result = (out_list->free_chunks >= chunks_needed ? RET_OK : RET_NO_SPACE);
  }
...
handle_result_and_exit(int result,
  ...
  const char* partition = (partition2 != NULL ? partition2 : "unknown partition");
...
int main(int argc, char* argv[]) {
  ...
  if (argc > 1) {
    ...
    if (0)
    ...
    else if (caseless_compare(command_name, CMD_HDL_INJECT_CD) || caseless_compare(command_name, CMD_HDL_INJECT_DVD)) { /* inject game image into a new HDL partition */
      int slice_index = -1;
      ...
      for (i = 5; i < argc; ++i) {
        ...
        else if (argv[i][0] == '+' || (argv[i][0] == '0' && argv[i][1] == 'x'))
          /* compatibility flags */
      ...
      if (compat_flags == COMPAT_FLAGS_INVALID || !(slice_index >= -1 && slice_index <= 1) || (dma == 0)) {
        show_usage_and_exit(argv[0], command_name);
      }
      ...
      for (i = 4; i < argc; ++i) {
        if (argv[i][0] == '+' || (argv[i][0] == '0' && argv[i][1] == 'x'))
          /* compatibility flags */
      ...
      if (new_name == NULL && new_dma == 0 && new_flags == COMPAT_FLAGS_INVALID && is_hidden == -1)
