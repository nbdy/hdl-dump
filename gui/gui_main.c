...
dlg_init_contents_list(HWND dlg) {
  ...
  ListView_SetExtendedListViewStyle(lvw,
                                    ListView_GetExtendedListViewStyle(lvw) | LVS_EX_FULLROWSELECT | LVS_EX_GRIDLINES);
    ...
    if (result == RET_OK) {
      ...
      if (result == RET_OK) { /* show free space info */
        ...
        for (i = 0; i < games_->count; ++i) {
          ...
          lv.iImage = (!game->is_dvd ? 0 : game->raw_size_in_kb / 1024 <= 4608 ? 1
                                                                               : 2);
...
install(HWND dlg) {
  ...
  if (strlen(target) > 0 && strlen(game_name) > 0 && strlen(signature) > 0 && strlen(source) > 0) {
    iin_t* iin;
    ...
    if (result != RET_OK && result != RET_INTERRUPTED)
