  ...
  *handle = CreateFile(path, GENERIC_WRITE, FILE_SHARE_READ, NULL,
  ...
  if (*handle != INVALID_HANDLE_VALUE) {
    if (estimated_size > 0) {
      ...
      if (SetFilePointerEx(*handle, output_size, NULL, FILE_BEGIN) && SetEndOfFile(*handle) && SetFilePointerEx(*handle, zero, NULL, FILE_BEGIN))
        return (OSAL_OK);
      else {
        CloseHandle(*handle);
        DeleteFile(path);
...
int osal_munmap(osal_mmap_t* mm) {
  if (UnmapViewOfFile(mm->start) && CloseHandle(mm->mmap)) {
    osal_free(mm);
    return (OSAL_OK);
  }
  return (OSAL_ERR);
...
int osal_query_hard_drives(osal_dlist_t** hard_drives) {
  ...
  *hard_drives = NULL;
  ...
  if (result != RET_OK && *hard_drives != NULL)
...
int osal_query_optical_drives(osal_dlist_t** optical_drives) {
  ...
  *optical_drives = NULL;
  ...
  if (result != RET_OK && *optical_drives != NULL)
